...
if (mysqli_query($app,"SELECT place_id,image FROM parkplace WHERE place_id IN(SELECT place FROM parking)"))
{
    ...
    while ($row = mysqli_fetch_assoc($sale)) {
        $image = base64_encode($row['image']);
        echo "<button style=background-color:red><a href=order/order.php style=font-size:50px;color:black;pointer-events:none;cursor:default> <img src=data:image/jpeg;base64,". $image ." style=width:450px>". $row['place_id'] ."</a></button>";
     }
}
if (mysqli_query($app,"SELECT place_id,image FROM parkplace WHERE place_id NOT IN(SELECT place FROM parking)") or $count == 0)
{
    ...
    while ($row = mysqli_fetch_assoc($free)) {
    $image = base64_encode($row['image']);
    echo "<button style=background-color:aqua><a href=order/order.php style=font-size:50px;color:black> <img src=data:image/jpeg;base64,". $image ." style=width:450px>". $row['place_id'] ."</a></button>";
 }
}
?>
</main>
